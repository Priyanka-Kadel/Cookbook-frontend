  ...
  useEffect(() => {
    const fetchUserRooms = async () => {
      try {
        ...
        console.log('Fetching user rooms with token:', token ? 'Token exists' : 'No token');
        ...
        console.log('User rooms response status:', response.status);
        ...
        if (response.ok) {
          ...
          console.log('User rooms filtered:', userRooms);
          setFlats(userRooms);
        } else {
  ...
  useEffect(() => {
    const fetchUserRoommates = async () => {
      try {
        ...
        console.log('Fetching user roommates with token:', token ? 'Token exists' : 'No token');
        ...
        console.log('User roommates response status:', response.status);
        
        if (response.ok) {
          const data = await response.json();
          console.log('All roommates data:', data);
          ...
          console.log('User roommates filtered:', userRoommates);
    ...
    if (window.confirm("Are you sure you want to delete this roommate listing?")) {
      try {
        ...
        if (response.ok) {
          ...
          alert("Roommate listing deleted successfully");
        } else {
          alert("Failed to delete roommate listing");
        }
      } catch (error) {
