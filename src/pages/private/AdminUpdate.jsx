...
const AdminUpdate = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log('Is roommate:', isRoommate);
        ...
        if (isRoommate && roommate) {
          ...
          console.log('Using room data from state:', flat);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = getAuthToken();
      console.log('Update token:', token ? 'Token exists' : 'No token');
      ...
      console.log('Is roommate update:', isRoommate);
      ...
      if (response.ok) {
        ...
        alert(`${isRoommate ? 'Roommate' : 'Room'} updated successfully`);
        navigate("/adminDash");
      } else {
        ...
        if (response.status === 500) {
          ...
          alert(`Not found (404). The ${isRoommate ? 'roommate' : 'room'} might not exist.`);
        } else {
          alert(`Failed to update ${isRoommate ? 'roommate' : 'room'}. Status: ${response.status}\n\nError: ${errorData}`);
        }
      }
    } catch (error) {
