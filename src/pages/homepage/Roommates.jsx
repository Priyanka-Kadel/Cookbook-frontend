...
const Roommates = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-8 text-blue-900">Available Roommates</h1>
      {/* Filter Bar */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredRoommates.length > 0 ? (
          filteredRoommates.map((mate) => (
            <div key={mate._id} className="bg-white p-4 rounded-lg shadow-md border border-blue-100">
              <img 
                src={mate.roommateImage ? `http://localhost:3000/${mate.roommateImage}` : '/default-avatar.png'} 
                alt="Roommate" 
                className="w-full h-48 object-cover rounded mb-3" 
              />
              <h2 className="text-lg font-semibold text-blue-900 mb-1">{mate.name}</h2>
              <p className="text-blue-700 text-sm">Gender: {mate.gender}</p>
              <p className="text-blue-700 text-sm">Age: {mate.age}</p>
              <p className="text-blue-700 text-sm">Budget: ₹{mate.budget}</p>
              <p className="text-blue-700 text-sm">Location: {mate.preferredLocation}</p>
              <Link to={`/roommate-details/${mate._id}`} className="block mt-2 bg-blue-600 hover:bg-blue-700 text-white text-center py-2 rounded font-bold">View Details</Link>
            </div>
          ))
        ) : (
          <p className="col-span-full text-center text-blue-700">No roommates available.</p>
        )}
      </div>
    </div>
  );
